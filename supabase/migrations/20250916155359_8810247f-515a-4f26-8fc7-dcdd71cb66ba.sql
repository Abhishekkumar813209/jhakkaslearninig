...
-- Seed 10 demo students in profiles (ids fixed for consistency)
WITH s AS (
  SELECT * FROM (VALUES
...
WITH a AS (
  ...
  ) AS t(student_id, total_study_time_minutes, average_score, streak_days, batch_rank, overall_rank)
)
...
SELECT student_id, total_study_time_minutes, average_score, streak_days, batch_rank, overall_rank
FROM a
