  ...
  "extracted_questions": [
    {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              { text: systemPrompt + '\n\nExtract all questions from this image and return them in JSON format.' },
              {
                inline_data: {
                  mime_type: mimeType,
                  data: base64Data
...
Analyze and suggest the BEST game type for maximum learning engagement.`;
      ...
      const suggestionResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const suggestion = JSON.parse(suggestionData.candidates?.[0]?.content?.parts?.[0]?.text);
