-- Create unique index on gamified_exercises to prevent duplicates
CREATE UNIQUE INDEX IF NOT EXISTS idx_gamified_exercises_unique 
ON gamified_exercises(topic_content_id, game_order);

-- Backfill gamified_exercises from topic_learning_content where lesson_type='game'
-- Use ROW_NUMBER to ensure unique game_order per topic_content_id
WITH numbered_games AS (
  SELECT 
    tcm.id as topic_content_id,
    tlc.game_data,
    tlc.xp_reward,
    tlc.coin_reward,
    ROW_NUMBER() OVER (PARTITION BY tcm.id ORDER BY tlc.content_order, tlc.id) as game_order
  FROM topic_learning_content tlc
  JOIN topic_content_mapping tcm ON tcm.topic_id = tlc.topic_id AND tcm.content_type = 'theory'
  WHERE tlc.lesson_type = 'game'
    AND tlc.human_reviewed = true
    AND tlc.game_data IS NOT NULL
)
INSERT INTO gamified_exercises (
  topic_content_id,
  exercise_type,
  exercise_data,
  correct_answer,
  explanation,
  difficulty,
  xp_reward,
  coin_reward,
  game_order
)
SELECT 
  topic_content_id,
  'mcq'::exercise_type as exercise_type,
  jsonb_build_object(
    'question', game_data->>'question',
    'options', game_data->'options',
    'correctAnswerIndex', COALESCE((game_data->>'correct_answer')::int, 0),
    'marks', COALESCE((game_data->>'marks')::int, 1),
    'difficulty', COALESCE(game_data->>'difficulty', 'medium')
  ) as exercise_data,
  jsonb_build_object(
    'correctAnswerIndex', COALESCE((game_data->>'correct_answer')::int, 0)
  ) as correct_answer,
  game_data->>'explanation' as explanation,
  COALESCE(game_data->>'difficulty', 'medium') as difficulty,
  COALESCE(xp_reward, 10) as xp_reward,
  COALESCE(coin_reward, 2) as coin_reward,
  game_order
FROM numbered_games
ON CONFLICT (topic_content_id, game_order) DO UPDATE SET
  exercise_data = EXCLUDED.exercise_data,
  correct_answer = EXCLUDED.correct_answer,
  explanation = EXCLUDED.explanation,
  difficulty = EXCLUDED.difficulty,
  xp_reward = EXCLUDED.xp_reward,
  coin_reward = EXCLUDED.coin_reward;

-- Create trigger function to auto-sync games from topic_learning_content
CREATE OR REPLACE FUNCTION sync_gamified_exercises_from_content()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_mapping_id UUID;
  v_game_order INTEGER;
BEGIN
  -- Only process if lesson_type is 'game' and human_reviewed is true
  IF NEW.lesson_type = 'game' AND NEW.human_reviewed = true AND NEW.game_data IS NOT NULL THEN
    -- Get the topic_content_mapping id for this topic
    SELECT id INTO v_mapping_id
    FROM topic_content_mapping
    WHERE topic_id = NEW.topic_id AND content_type = 'theory'
    LIMIT 1;
    
    IF v_mapping_id IS NOT NULL THEN
      -- Calculate next game_order
      SELECT COALESCE(MAX(game_order), 0) + 1 INTO v_game_order
      FROM gamified_exercises
      WHERE topic_content_id = v_mapping_id;
      
      -- Upsert the gamified_exercise
      INSERT INTO gamified_exercises (
        topic_content_id,
        exercise_type,
        exercise_data,
        correct_answer,
        explanation,
        difficulty,
        xp_reward,
        coin_reward,
        game_order
      ) VALUES (
        v_mapping_id,
        'mcq'::exercise_type,
        jsonb_build_object(
          'question', NEW.game_data->>'question',
          'options', NEW.game_data->'options',
          'correctAnswerIndex', COALESCE((NEW.game_data->>'correct_answer')::int, 0),
          'marks', COALESCE((NEW.game_data->>'marks')::int, 1),
          'difficulty', COALESCE(NEW.game_data->>'difficulty', 'medium')
        ),
        jsonb_build_object(
          'correctAnswerIndex', COALESCE((NEW.game_data->>'correct_answer')::int, 0)
        ),
        NEW.game_data->>'explanation',
        COALESCE(NEW.game_data->>'difficulty', 'medium'),
        COALESCE(NEW.xp_reward, 10),
        COALESCE(NEW.coin_reward, 2),
        v_game_order
      )
      ON CONFLICT (topic_content_id, game_order) DO UPDATE SET
        exercise_data = EXCLUDED.exercise_data,
        correct_answer = EXCLUDED.correct_answer,
        explanation = EXCLUDED.explanation,
        difficulty = EXCLUDED.difficulty,
        xp_reward = EXCLUDED.xp_reward,
        coin_reward = EXCLUDED.coin_reward;
    END IF;
  END IF;
  
  RETURN NEW;
END;
$$;

-- Create trigger to auto-sync on INSERT/UPDATE
DROP TRIGGER IF EXISTS sync_games_on_content_change ON topic_learning_content;
CREATE TRIGGER sync_games_on_content_change
  AFTER INSERT OR UPDATE ON topic_learning_content
  FOR EACH ROW
  EXECUTE FUNCTION sync_gamified_exercises_from_content();