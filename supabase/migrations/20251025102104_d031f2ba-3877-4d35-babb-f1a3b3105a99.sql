-- Drop and recreate calculate_topic_status function - purely game-based status
...
DECLARE
  ...
  v_status TEXT := 'grey';
BEGIN
  -- Get game progress data with fully qualified column names
  SELECT 
  ...
  -- Set test average to 0 (tests are not linked to specific topics)
  v_test_avg_score := 0;

  -- Determine status PURELY based on game completion rate (no test dependency)
...
-- Recalculate all existing topic statuses with the new logic
DO $$
DECLARE
  rec RECORD;
  calc_result RECORD;
