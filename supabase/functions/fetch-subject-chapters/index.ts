...
serve(async (req) => {
  ...
  try {
    const { exam_type, subject, student_class, board, fetch_mode = 'initial', already_fetched = [] } = await req.json();
...
- Only return chapter names, NOT topics`;
    ...
    if (fetch_mode === 'remaining') {
      userPrompt = `Generate REMAINING chapters for:\nExam Type: ${exam_type}\nSubject: ${subject}\n\nAlready fetched chapters: ${already_fetched.join(', ')}\n\nGenerate 15-25 additional important chapters NOT in the above list.`;
      maxTokens = 3000;
    } else {
      userPrompt = `Generate first 20-25 chapters for:\nExam Type: ${exam_type}\nSubject: ${subject}\n\nFocus on the most important foundational chapters first.`;
    }
