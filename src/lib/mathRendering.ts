 ...
 * Simple Syntax: [img:URL:width:align:padding]
 ...
 * 
 * Backward compatible with old extended format but only uses simple positioning
 */
function expandImageTokens(text: string): string {
  return text.replace(/\[img:([^\]]+)\]/g, (_, params) => {
    ...
    return `<img src="${url}" class="block ${alignClass} h-auto my-2" style="${style}" alt="Question image" />`;
  });
}
