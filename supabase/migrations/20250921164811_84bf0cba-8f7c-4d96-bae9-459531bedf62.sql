...
-- Add new columns to existing lectures table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lectures' AND column_name = 'playlist_id') THEN
    ALTER TABLE public.lectures ADD COLUMN playlist_id UUID REFERENCES public.playlists(id) ON DELETE CASCADE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'lectures' AND column_name = 'thumbnail_url') THEN
    ALTER TABLE public.lectures ADD COLUMN thumbnail_url TEXT;
  END IF;
END $$;

-- Add new columns to existing video_progress table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'video_progress' AND column_name = 'playlist_id') THEN
    ALTER TABLE public.video_progress ADD COLUMN playlist_id UUID;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'video_progress' AND column_name = 'student_id') THEN
    ALTER TABLE public.video_progress ADD COLUMN student_id UUID NOT NULL DEFAULT auth.uid();
  END IF;
END $$;
